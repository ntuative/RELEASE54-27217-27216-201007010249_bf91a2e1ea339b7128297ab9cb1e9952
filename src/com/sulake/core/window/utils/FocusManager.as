package com.sulake.core.window.utils
{
   ...
   public class FocusManager extends ChildEntityArrayReader implements IWindowFocusManagerService
   {
      ...
      public function FocusManager(param1:DisplayObject)
      {
         ...
         var_212.addEventListener(Event.ACTIVATE,onActivateEvent);
         var_212.addEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
         var_212.addEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
         var_212.addEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
         super();
      }
      ...
      public function dispose() : void
      {
         if(!_disposed)
         {
            var_212.removeEventListener(Event.ACTIVATE,onActivateEvent);
            var_212.removeEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
            var_212.removeEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
            var_212.removeEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
            var_212 = null;
            _disposed = true;
            var_34 = null;
         }
