package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_406 >= var_308.length)
         {
            return "";
         }
         var petColor:IPetColor = var_308[var_406] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_601[var_1512] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_406 = param1.index;
         if(var_406 < 0 || var_406 > var_308.length)
         {
            var_406 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_646.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         var_850 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_308 = [];
         var_601 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_850,this);
               if(_loc11_)
               {
                  var_601 = _loc11_.breeds;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_601.length)
         {
            _loc12_ = var_601[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_850,_loc12_.id),getRaceLocalizationKey(var_850,_loc12_.id)));
            _loc6_++;
         }
         var_646.add(var_850,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_907,onApproveNameResult);
         _loc5_ = var_646.getWithIndex(0);
         if(_loc5_ != null)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_646.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_406 >= 0 && var_406 < var_308.length)
         {
            _loc7_ = var_308[var_406] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_601[var_1512] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_850,_loc3_.id,_loc2_,AvatarScaleType.const_47,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_277,AvatarAction.const_660);
