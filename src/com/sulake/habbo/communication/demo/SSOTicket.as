package com.sulake.habbo.communication.demo
{
   ...
   public class SSOTicket extends EventDispatcher
   {
      ...
      public static const const_340:String = "success";
      ...
      private var var_1348:IHabboWebLogin;
      ...
      public function SSOTicket(param1:IAssetLibrary, param2:IHabboWebLogin, param3:String)
      {
         ...
         _assets = param1;
         var_1348 = param2;
         var_1347 = "http://" + param3 + "/client";
         var_1348.init();
         var_1348.addEventListener(HabboWeb.const_220,requestClientURL);
      }
      ...
      {
         var _loc2_:String = var_1347;
         if(_assets.hasAsset(_loc2_))
         {
            Logger.log("[CoreLocalizationManager] reload localization for url: " + var_1347);
         }
         var _loc3_:URLRequest = new URLRequest(var_1347);
         if(_assets.hasAsset(_loc2_))
         {
      ...
      private function clientURLComplete(param1:Event = null) : void
      {
         ...
         {
            var_1348.requestReAuthenticate();
         }
         else
         {
            try
            {
               var_1613 = /\"sso.ticket\" : \"(.*?)\"/.exec(data)[1];
               var_2083 = /\"flash.client.url\" : \"(.*?)\"/.exec(data)[1];
               if(false)
               {
                  dispatchEvent(new Event(const_340));
               }
               else
               {
