package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         _loc6_.addEventListener(AssetLoaderEvent.const_41,onConfigurationError);
         var_825 = new Timer(DOWNLOAD_TIMEOUT,1);
         var_825.addEventListener(TimerEvent.TIMER_COMPLETE,onNextDownloadTimeout);
      }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in var_823)
         {
            _loc7_ = true;
            _loc4_ = var_823[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete var_766[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete var_823[_loc3_];
         }
         while(_loc6_ < var_996.length)
         {
            _loc5_ = var_996[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               var_996.splice(_loc6_,1);
            }
            _loc6_++;
         }
         var_825.start();
      ...
      public function loadFigureSetData(param1:AvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = var_766[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               var_766[_loc3_] = _loc6_;
            }
            var_823[_loc3_] = _loc4_;
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + var_997);
            if(var_1450.indexOf("?") > 0)
            {
               _loc2_ = var_1450 + "&retry=" + var_997;
            }
            else
            {
               _loc2_ = var_1450 + "?retry=" + var_997;
            ...
            _loc4_.addEventListener(AssetLoaderEvent.const_41,onConfigurationError);
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,var_2355,_assets);
      ...
      {
         var _loc1_:* = null;
         while(false && var_996.length < const_1457)
         {
            _loc1_ = var_680[0];
            _loc1_.startDownloading();
            var_996.push(var_680.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(var_680.indexOf(param1) == -1 && !param1.isReady)
         {
            var_680.push(param1);
            processPending();
         }
      ...
      private function onConfigurationComplete(param1:Event) : void
      {
         ...
         generateMap(data);
         var_1704 = true;
         for each(initListener in var_824)
         {
