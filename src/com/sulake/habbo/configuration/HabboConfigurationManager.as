package com.sulake.habbo.configuration
{
   ...
   public class HabboConfigurationManager extends Component implements IHabboConfigurationManager
   {
      ...
      private static var var_1584:String = "%";
      ...
      public function HabboConfigurationManager(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null)
      {
         ...
         var_530 = getKey("url_prefix");
         if(var_530 != null && false)
         {
            setKey("client.fatal.error.url",var_530 + "/flash_client_error");
         }
      ...
      private function onConfigurationError(param1:Event = null) : void
      {
         ...
         Core.error("Could not load external variables. HTTP status " + _loc3_ + ". Client startup failed!",true,Core.const_1342);
      }
      ...
      private function parseConfiguration(param1:String) : void
      {
         ...
         if(getKey("local.environment") == "FurniturePartyBeamerVisualization")
         {
            setKey("url.prefix",getKey("url.prefix.local"));
         }
         var_530 = getKey("url.prefix");
         if(false)
         {
            _loc9_ = var_530.replace("http://","");
      ...
      private function onInitConfiguration(param1:Event = null) : void
      {
         ...
         var_959 = true;
         events.dispatchEvent(new Event(HabboConfigurationEvent.const_155));
         if(var_2053)
         {
      ...
      public function getKey(param1:String, param2:String = "", param3:Dictionary = null) : String
      {
         ...
         if(_loc4_ != null)
         {
            ...
            {
               _loc4_ = _loc4_.replace("%predefined%",var_530);
      ...
      private function fillParams(param1:String, param2:Dictionary) : String
      {
         ...
         while(_loc3_ < 10)
         {
            _loc4_ = param1.indexOf(var_1584);
            if(_loc4_ < 0)
            {
               break;
            }
            _loc5_ = param1.indexOf(var_1584,_loc4_ + 1);
            if(_loc5_ < 0)
            {
               break;
            }
